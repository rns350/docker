...
help=$(
  cat <<'HEREDOC'
-----------------------------------------------------------------------------
  This script updates the version.txt and the Docker image.
-----------------------------------------------------------------------------
 - Options:
...
version_LATEST=$(docker run --rm "$NAME_TAG_IMAGE_TEST" sqlite3 --version)
VERSION_OS_LATEST=$(docker run --rm "$NAME_TAG_IMAGE_TEST" cat /etc/alpine-release)

echoGreen "- Previous SQLite3 version was: ${version}"
echoGreen "- Current  SQLite3 version is : ${version_LATEST}"
...
version_SHORT=$(echo "$version_LATEST" | awk '{print $1}')

# Update version (overwrites the version file)
echo "version='${version_LATEST}'" >"$NAME_FILE_VERSION" || {
