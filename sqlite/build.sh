...
# Include previous version of SQLite3 (version)
# shellcheck source=./version.txt
source "$NAME_FILE_VERSION"

# Get current short version of SQLite3
version_SHORT=$(echo "$version" | awk '{print $1}')
NAME_IMAGE_LATEST="${NAME_IMAGE}:latest"
NAME_IMAGE_VERSIONED="${NAME_IMAGE}:${version_SHORT}"
...
#  Main
# =============================================================================
echoGreen "- Building image with SQLite3 version: ${version_SHORT}"
echo "  ${version}"
...
echo "$@" | grep 'buildx' >/dev/null || {
  echoGreen "- Building latest image (${NAME_IMAGE_LATEST}) and versioned tagged image ($NAME_IMAGE_VERSIONED)..."
  docker build . --tag "$NAME_IMAGE_LATEST" --tag "$NAME_IMAGE_VERSIONED".
}
...
echoGreen "Pushing to github"
docker image push "$NAME_IMAGE_LATEST"
docker image push "$NAME_IMAGE_VERSIONED"
